...
const basketSlice = createSlice({
  ...
  reducers: {
    ...
    addToBasket: (state, action) => {
      console.log("ADD TO BSKET IS BEING CALLED");
    ...
    addToLocalBasket: (state, action) => {
      if (state.localBasket.find((item) => item.productSlug === action.payload.product.slug)) {
        // We add the quantity of the product to the existing quantity of the product in the basket
        state.localBasket.find((item) => item.productSlug === action.payload.product.slug).quantity += action.payload.quantity;
      } else {
        // We add the product to the basket
    ...
    removeFromLocalBasket: (state, action) => {
      state.localBasket = state.localBasket.filter((item) => item.productSlug !== action.payload.product.slug);
...
export const getBasketProduct = (productSlug, quantity) => {
  return async dispatch => {
    const token = Cookies.get("YQM-token");

    dispatch(
      basketSlice.actions.showNotifications({
        status: "pending",
        title: "Sending...",
    ...
    try {
      ...
      dispatch(basketSlice.actions.addToBasket({ product: product, quantity: quantity }));
      dispatch(basketSlice.actions.showNotifications({ status: "success", title: "Success", message: "Successfully fetched product with slug ${productSlug}", flash_code: "GET_SHOW_PRODUCT_SUCCESS", slice: "product" }));
    } catch (error) {
      console.error(error);
      dispatch(basketSlice.actions.showNotifications({ status: "error", title: "Error", message: "Failed to fetch product with slug ${productSlug}", flash_code: "GET_SHOW_PRODUCT_ERROR", slice: "product" }));
    }
  };
};
