...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    setProductsToWish: (state, action) => {
      state.productsToWish = action.payload;
    },
...
export const getForWishingProducts = () => {
  return async dispatch => {
    ...
    const sendRequest = async () => {
      const res = await fetch(`${baseEndpointUrl}/api/v1/products/for_wishing`, {
        method: "GET",
        headers: {
    ...
    try {
      ...
      dispatch(productSlice.actions.setProductsToWish(products));
      dispatch(
        productSlice.actions.showNotifications({
          status: "success",
          title: "Success",
...
export const getForOrderingProducts = () => {
  return async dispatch => {
    const token = Cookies.get("YQM-token");
    ...
    const sendRequest = async () => {
      const res = await fetch(`${baseEndpointUrl}/api/v1/products/for_ordering`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          // Authorization: `Bearer ${token}`,
    ...
    try {
      ...
      dispatch(productSlice.actions.setProductsToOrder(products));
      dispatch(
        productSlice.actions.showNotifications({
          status: "success",
          title: "Success",
