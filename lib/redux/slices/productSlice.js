...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    setProductsToWish: (state, action) => {
      ...
      console.log(action.payload.maxNumberOfForWishingProducts);
...
export const getForWishingProducts = (pageNumber) => {
  return async dispatch => {
    ...
    const sendRequest = async () => {
      const res = await fetch(
        `${baseEndpointUrl}/api/v1/products/for_wishing?page=${pageNumber}`,
        {
          method: "GET",
          headers: {
