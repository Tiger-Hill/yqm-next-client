...
export const initialState = {
  token: Cookies.get("SESAME-token") ? Cookies.get("SESAME-token") : null,
  user: Cookies.get("SESAME-user") ? JSON.parse(Cookies.get("SESAME-user")) : null, // ! If there's a token, there's a user obviously
  isLoggedIn: Cookies.get("SESAME-token") ? true : false,
  userType: Cookies.get("SESAME-user") ? JSON.parse(Cookies.get("SESAME-user")).user_type : "Anonymous",
  userDetailsSlug: Cookies.get("SESAME-userDetailsSlug") ? Cookies.get("SESAME-userDetailsSlug") : null,
  ...
  reducers: {
    ...
    setSession: (state, action) => {
      ...
      if (action.payload.isRememberMeChecked) {
        ...
        Cookies.set("SESAME-token", JSON.stringify(action.payload.token).slice(1, -1), { secure: true });
        Cookies.set("SESAME-tokenExpirationTime", sameTimeTomorrow, { secure: true });
        Cookies.set("SESAME-rememberMe", JSON.stringify(action.payload.isRememberMeChecked), { secure: true });
        Cookies.set("SESAME-userEmail", action.payload.user.email, { secure: true });
      } else {
        Cookies.set("SESAME-token", JSON.stringify(action.payload.token).slice(1, -1), { secure: true });
        Cookies.set("SESAME-rememberMe", action.payload.isRememberMeChecked, { secure: true });
      }

      Cookies.set("SESAME-user", JSON.stringify(action.payload.user), { secure: true, expires: 1 });
      Cookies.set("SESAME-userDetailsSlug", action.payload.userDetailsSlug, { secure: true, expires: 1 });
    },
    ...
    destroySession: (state) => {
      ...
      // const isRememberMeChecked = !!Cookies.get("SESAME-rememberMe")
      // !isRememberMeChecked && Cookies.remove("SESAME-userEmail");

      Cookies.remove("SESAME-token");
      Cookies.remove("SESAME-tokenExpirationTime");
      Cookies.remove("SESAME-user");
      Cookies.remove("SESAME-userDetailsSlug");
    },
  },
});
