...
export const getAllOrdersCsv = () => {
  return async (dispatch) => {
    ...
    const sendRequest = async () => {
      ...
      dispatch(adminSlice.actions.showNotifications({ status: "success", title: "Success", message: "Successfully fetched all admin orders", flash_code: "GET_ALL_ORDERS_CSV_ADMIN", slice: "admin", }));
    } catch (error) {
      console.error(error);
      dispatch(adminSlice.actions.showNotifications({ status: "error", title: "Error", message: error.message, flash_code: "GET_ALL_ADMIN_ORDERS_ERROR", slice: "admin", }));
    }
