...
export const getAllOrdersCsv = () => {
  return async (dispatch) => {
    ...
    const sendRequest = async () => {
      ...
      // ! This means there is no order in the backend to render as CSV
      if (res.status === 422) {
        dispatch(adminSlice.actions.showNotifications({ status: "error", title: "Error", message: "No order data", flash_code: "GET_ALL_ORDERS_CSV_ADMIN_NO_DATA_ERROR", slice: "admin" }));
        return
      }
...
export const getAllOrdersCsvForProduct = (productSlug) => {
  return async (dispatch) => {
    ...
    const sendRequest = async () => {
      ...
      // ! This means there is no order for the given product in the backend to render as CSV
      if (res.status === 422) {
        dispatch(adminSlice.actions.showNotifications({ status: "error", title: "Error", message: "No order data", flash_code: "GET_ALL_ORDERS_FOR_PRODUCT_CSV_ADMIN_NO_DATA_ERROR", slice: "admin" }));
        return
      }
...
export const checkUpdateOrdersCsv = (targetStatus, csvFile) => {
  return async (dispatch) => {
    ...
    const sendRequest = async () => {
      ...
      //   rowData.attributes = row.attributes
      //   rowData.user = row.relationships.user.data.id
      //   rowData.product = row.relationships.product.data.id

      //   orderRows.push(rowData)
      // })
